...
def main():
    tests = pd.read_csv(OUT / "tables/nonparametric_same_month_tests.csv")
    ...
    text = [
        f"""=== Nonparametric Results Across Historical Locations and Variables
...
across 2020–2025. There are {int(tests.level.sum())} detected rank differences and {int(tests.spread.sum())} detected spread
...
"""
    ...
    text.append("""  ),
  caption: [Number of months with detected rank / spread differences, out of
    twelve tested per cell. Ranks are tested with Kruskal–Wallis and spread with Fligner–Killeen scores.],
)
...
""")
    ...
    for site in sites:
        ...
        for batch in range(0, len(sub), 6):
            ...
            for ax, r in zip(axes.flat, rows.itertuples()):
                ...
                ax.set(
                    title=f"{label} · {calendar.month_abbr[r.month]}\nRank p={r.level_p_holm:.4f}; spread p={r.spread_p_holm:.4f}",
                    ylabel=unit,
                    xlabel="Year (20xx)",
                )
            ...
            for ax in list(axes.flat)[len(rows) :]:
                ax.axis("off")
            filename = f"nonparam_same_month_{site}_{batch // 6 + 1}.png"
    ...
    (REPORT / "nonparametric-same-month-results.typ").write_text("".join(text))
    print(
